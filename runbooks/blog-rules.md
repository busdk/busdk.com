...
Full article, style, language, verification, and iteration rules for the
blog; expands the root `AGENTS.md` core.
...
58. Use an explicit blog-iteration workflow for every substantial blog pass:
    1. update `PLAN.md` first with the current pass and any newly found article ideas
    ...
    7. if the review uncovers more promising topics or more writing fixes than can be completed immediately, add them back to `PLAN.md` as open items instead of leaving them implicit
